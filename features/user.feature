Feature: User API
  ...
  Scenario: I can get the list of Users
    Given I send a GET request to "/api/v1/users/" with the following:
      |||
    Then the response status should be "200"
    And the JSON response should have "$..guid" with a length: ">0"
  ...
  Scenario: I can't create a User without a name
    Given I send a POST request to "/api/v1/users/" with the following:
      | user |   {"name":""}   |
    Then the response status should be "400"
    And the JSON response should have "ERRS" field
  ...
  Scenario: I can't update a User without a name
    ...
    When I send a PUT request to "/api/v1/users/1111" with the following:
      | user |   {"name":""}   |
    Then the response status should be "400"
    And the JSON response should have text: "ERRS"
